...
def train(detector,train_dataloader,learning_rate=0.001):
    ...
    for d_x,d_y,p_x,p_y in train_dataloader:
        ...
        d_tmp_loss = d_criterion(d_output, d_y) #損失計算
        d_prediction = d_output.data.max(1)[1] #予測結果
        d_runnning_accu += d_prediction.eq(d_y).sum().item()/batch_size
...
def eval(detector,valid_dataloader):
    ...
    for d_x,d_y,p_x,p_y in valid_dataloader:
        ...
        for y_true,y_pred in zip(d_y,d_prediction):
...
for i in range(k_num):
    ...
    detector.load_state_dict(torch.load("/net/nfs2/export/home/ohno/CR_pytorch/Culmination/Learned_models/pre_trained_detector"))
    ...
    torch.save(detector.state_dict(), "/net/nfs2/export/home/ohno/CR_pytorch/Culmination/Learned_models/detector_k"+str(i+1))
