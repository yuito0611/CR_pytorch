...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        out_dis_val = torch.tensor(self.distanced_ten_hot_encoded_value[idx:idx+9]).to(self.device)
        out_val = torch.tensor(self.val_idx[idx:idx+9][0]).to(self.device)
        out_ans = torch.tensor(self.ans_idx[idx+8]).to(self.device)

        return out_dis_val,out_val,out_ans



class Proofreader(nn.Module):
    def __init__(self,input_size ,hidden_dim, class_num,n_layers,tokens,c2v):
    ...
    def forward(self, x,x_dis):
        ...
        x_m_vec = self.mean_vector[x] #5文字目の意味ベクトル
        ...
        out, hidden = self.rnn(x_dis, hidden)
        out = out[:,-1,:]
...
def train(model,train_dataloader,learning_rate=0.001):
    ...
    model.train()
    for i,(x_dis,x,y) in enumerate(train_dataloader):
        output = model(x,x_dis)
...
def eval(model,valid_dataloader,is_show_ans_pred=False):
    ...
    model.eval()
    for x_dis,x,y in valid_dataloader:
        output = model(x,x_dis)
        prediction = output.data.max(1)[1] #予測結果
        accuracy += prediction.eq(y.data).sum().item()/batch_size
...
for i in range(k_num):
    ...
    proofreader = Proofreader(VOCAB_SIZE,HIDDEN_SIZE,VOCAB_SIZE,1,tokens,char2vector)
