...
#候補文字10個をDTHE->最終層で結合することでattention、OCR第一候補のみで予測、7文字の中から中央１文字予測、embedding、
import torch
...
class OCR1Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self,index):
        input  = self.val_idx[index:index+7,0].to(device)
        target = self.ans_idx[index+3].to(device)
        distance = self.distanced_ten_hot_encoded_value[index+3].to(device)
